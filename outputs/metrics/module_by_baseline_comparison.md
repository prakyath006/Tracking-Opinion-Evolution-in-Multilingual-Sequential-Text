...
| :--- | :--- | :--- | :--- |
| **Full Model (OET)** | Frozen mBERT Encoder (0 trainable encoder layers) + Bi-LSTM + Self-Attention + Multi-Task Heads | Sequence (Multi-Task) | End-to-end multi-task with inverse-frequency loss weighting; 3,093,003 trainable params (Bi-LSTM + Attention + heads only) |
...
| **TextCNN** | Randomly-Initialised Embeddings (300d, learned from scratch) + 1D Convolutions (2, 3, 4, 5) + Max-Pool | Single Sentence | Non-transformer n-gram baseline |
...
| **Module 2: Encoder Backbone** | bert-base-multilingual-cased (0 layers trainable -- run used --freeze_encoder) | bert-base-multilingual-cased (0 layers trainable -- re-run 2026-09-05 at matched capacity, see docs/capacity_matched_comparison.md; superseded the original 2026-09-04 run's all-12-layers-trainable configuration) | xlm-roberta-base (SentencePiece 100-language model; 0 layers trainable -- re-run 2026-09-05 at matched capacity, see docs/capacity_matched_comparison.md) | bert-base-multilingual-cased | bert-base-multilingual-cased | Randomly-initialised nn.Embedding (300d), learned from scratch -- no pretrained vectors |
...
| — *Vocabulary Size* | 119,547 tokens (WordPiece multilingual) | 119,547 tokens | 250,002 tokens (BPE multilingual) | 119,547 tokens | 119,547 tokens | <=20,000 whitespace tokens built from the training split (build_vocab) |
| **Module 3: Sequential Architecture** | Bidirectional LSTM (2 layers, hidden=256) + Self-Attention | None (independent review classification) | None (independent review classification) | Bidirectional LSTM (final hidden state pooled, no attention) | Multi-Head Self-Attention directly over embedding sequence (no LSTM) | 1D Convolution over word tokens (kernel sizes 2, 3, 4, 5) |
...
- **mBERT vs. XLM-R**: Evaluated via `src/mlm_perplexity_eval.py`. XLM-R has a larger vocabulary (250K vs 119K) and better raw script coverage, but requires higher memory and slower throughput. The Full Model uses mBERT with a fully frozen encoder (0 trainable encoder layers) -- it does not fine-tune any encoder layers; only the Bi-LSTM, Attention, and classification heads (3,093,003 params) are trained.
